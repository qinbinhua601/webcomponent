...
class CodeBlock extends HTMLElement {
  ...
  _setupPreview() {
    ...
    preview?.addEventListener('click', () => {
      ...
      if (this._preview) {
        let previewContainer = this.shadowRoot.querySelector('.preview-container');
        const temp = document.createElement('div');
        temp.className = 'preview-container'
        previewContainer = temp
        previewContainer.innerHTML = `<mermaid-chart code="${encodeURI(this._content)}"></mermaid-chart>`;
        this.shadowRoot.appendChild(previewContainer);
        this.shadowRoot.querySelector('.code-content').style.display = 'none';
        preview.textContent = '代码';
      } else {
        const previewContainer = this.shadowRoot.querySelector('.preview-container');
        if (previewContainer) {
          previewContainer.remove();
          this.shadowRoot.querySelector('.code-content').style.display = '';
